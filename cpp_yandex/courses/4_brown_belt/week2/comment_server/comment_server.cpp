...
HttpResponse& HttpResponse::AddHeader(string name, string value) {
  headers_.push_back(HttpHeader{ move(name), move(value) });
  return *this;
}
...
public:
  HttpResponse ServeRequest(const HttpRequest& req) {
    if (req.method == "POST") {
      ...
      if (req.path == "/user_comments") {
        return GetUserComment(req);
      } else if (req.path == "/captcha") {
        return GetCaptcha(req);
      } else {
        return HttpResponse(HttpCode::NotFound);
      }
    }
  ...
  HttpResponse ServeAddUser(const HttpRequest& req) {
  ...
  HttpResponse GetUserComment(const HttpRequest& req) {
  ...
  HttpResponse GetCaptcha(const HttpRequest& req) {
    return HttpResponse(HttpCode::Ok)

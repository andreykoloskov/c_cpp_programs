...
vector<Spending> LoadFromJson(istream& input) {
  ...
  auto& children = root.AsArray();
  for (const auto& child : children) {
    auto& res = child.AsMap();
    string category = res.at("category").AsString();
    int amount = res.at("amount").AsInt();
    result.push_back({ move(category), amount });
  }

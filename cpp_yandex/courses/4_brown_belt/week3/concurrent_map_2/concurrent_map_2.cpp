...
public:
  ...
  struct WriteAccess {
    ...
    WriteAccess(const K& key, LockMapType& bucket_content)
      : guard(bucket_content.mtx)
      , ref_to_value(bucket_content.mapping[key])
    {
    }
  };
  ...
  struct ReadAccess {
    ...
    ReadAccess(const K& key, const LockMapType& bucket_content)
      : guard(bucket_content.mtx)
      , ref_to_value(bucket_content.mapping.at(key))
    {
    }
  };
  ...
  WriteAccess operator[](const K& key) {
    auto& bucket = data[Abs(hasher(key)) % data.size()];
    return { key, bucket };
  }

  ReadAccess At(const K& key) const {
    const auto& bucket = data[Abs(hasher(key)) % data.size()];
    return { key, bucket };
  }
  ...
  MapType BuildOrdinaryMap() const {
    MapType result;
    for (auto& d : data) {
      auto g = lock_guard<mutex>(d.mtx);
      result.insert(begin(d.mapping), end(d.mapping));
    }
    return result;
  }

...
void Test1() {
  ...
  ASSERT_EQUAL(Split("a")[0], "a");

  ASSERT_EQUAL(Split("a b").size(), 2u);
  ASSERT_EQUAL(Split("a b")[0], "a");
  ASSERT_EQUAL(Split("a b")[1], "b");

  ASSERT_EQUAL(Split("a b c").size(), 3u);
  ASSERT_EQUAL(Split("a b c")[0], "a");
  ASSERT_EQUAL(Split("a b c")[1], "b");
  ASSERT_EQUAL(Split("a b c")[2], "c");
}
...
void Test2() {
  ...
  }
  {
    Domain domain("ya");
    auto res = domain.GetReversedParts();
    ASSERT_EQUAL(*res.begin(), "ya");
  }
  {
    Domain domain("ya.com");
    auto res = domain.GetReversedParts();
    ASSERT_EQUAL(*res.begin(), "com");
    ASSERT_EQUAL(*next(res.begin()), "ya");
  }
  {
    Domain domain("mail.ya.com");
    auto res = domain.GetReversedParts();
    ASSERT_EQUAL(*res.begin(), "com");
    ASSERT_EQUAL(*next(res.begin()), "ya");
    ASSERT_EQUAL(*next(next(res.begin())), "mail");
  }
}

void Test3() {
  ASSERT(IsSubdomain(Domain(""), Domain("")));
  ASSERT(IsSubdomain(Domain("ya"), Domain("ya")));
  ASSERT(IsSubdomain(Domain("ya.com"), Domain("ya.com")));
  ASSERT(IsSubdomain(Domain("mail.ya.com"), Domain("mail.ya.com")));
}

void Test4() {
  ASSERT_EQUAL(IsSubdomain(Domain("mail.ya.com"), Domain("ya.com")), true);
  ASSERT_EQUAL(IsSubdomain(Domain("ya.com"), Domain("mail.ya.com")), false);
}

void Test5() {
  vector<Domain> domains = { Domain("b.c"), Domain("a.b.c") };
  DomainChecker checker(domains.begin(), domains.end());
  ASSERT(checker.IsSubdomain(Domain("d.b.c")));
}

void Test6() {
  vector<Domain> banned_domains = { Domain("ya.ru") };
  vector<Domain> banned_to_check = { Domain("mail.ya.ru") };
  ASSERT(CheckDomains(banned_domains, banned_to_check)[0] == false);
}
...
void Test8() {
  {
    ...
    ASSERT_EQUAL(ReadDomains(in).size(), 0);
  }
  {
    istringstream in(R"(1
    ya.ru
    )");
    auto res = ReadDomains(in);
    ASSERT_EQUAL(res.size(), 1u);
    ASSERT_EQUAL(res.front(), Domain("ya.ru"));
  }
  {
    ...
    ASSERT_EQUAL(res.front(), Domain("ya.ru"));
    ASSERT_EQUAL(res.back(), Domain("mail.ru"));
  }
}

...
array<double, VERTEX_COUNT> tree_values, tree_add, tree_factor, tree_sub, tree_sub_values;
...
void Push(size_t v, size_t l, size_t r) {
  for (size_t w = v * 2; w <= v * 2 + 1; ++w) {
    if (w < VERTEX_COUNT) {
      ...
      tree_sub_values[w] += tree_sub[v] * (r - l) / 2;
    }
  }
  tree_factor[v] = 1;
  tree_add[v] = 0;
...
double ComputeSum(size_t v, size_t l, size_t r, size_t ql, size_t qr) {
  ...
  Push(v, l, r);
  if (ql <= l && r <= qr) {
    return tree_values[v] - tree_sub_values[v];
  }
...
void Sub(size_t v, size_t l, size_t r, size_t ql, size_t qr, double value) {
  ...
  if (ql <= l && r <= qr) {
    tree_sub[v] += value;
    tree_sub_values[v] += value * (r - l);
    return;
  }
  Sub(v * 2, l, (l + r) / 2, ql, qr, value);
  Sub(v * 2 + 1, (l + r) / 2, r, ql, qr, value);
  tree_sub_values[v] =
      (v * 2 < VERTEX_COUNT ? tree_sub_values[v * 2] : 0)
      + (v * 2 + 1 < VERTEX_COUNT ? tree_sub_values[v * 2 + 1] : 0);
}

void Multiply(size_t v, size_t l, size_t r, size_t ql, size_t qr, double tax) {
  if (v >= VERTEX_COUNT || qr <= l || r <= ql) {
    return;
  }
  Push(v, l, r);
  if (ql <= l && r <= qr) {
    tree_factor[v] *= ((100.0 - tax) / 100.0);
    tree_add[v] *= ((100.0 - tax) / 100.0);
    tree_values[v] *= ((100.0 - tax) / 100.0);
    return;
  }
  Multiply(v * 2, l, (l + r) / 2, ql, qr, tax);
  Multiply(v * 2 + 1, (l + r) / 2, r, ql, qr, tax);
  tree_values[v] =
      (v * 2 < VERTEX_COUNT ? tree_values[v * 2] : 0)
      + (v * 2 + 1 < VERTEX_COUNT ? tree_values[v * 2 + 1] : 0);
}

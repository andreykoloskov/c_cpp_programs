...
void SearchServer::AddQueriesStream(istream& query_input, ostream& search_results_output)
{
    ...
    for (string current_query; getline(query_input, current_query); ) {
        ...
        if (k > page_cnt) {
            it1 = result.begin() + k_all - k;
            auto it2 = result.begin() + k_all - 1;
    ...
    //size_t page_cnt = result.size() / 6 + ((result.size() % 6) ? 1 : 0);
    //auto it1 = result.begin();
    ...
    it1 = result.begin();
    auto it2 = (result.end() - it1 >= page_cnt) ? it1 + page_cnt : result.end();

    for (size_t i = 0; i < futures.size(); ++i) {
        futures[i].get();

        for (auto it = it1; it != it2; ++it) {
            search_results_output << it->current_query << ':';
            for (size_t i = 0; i < it->quantity; ++i) {
                search_results_output << " {"
                    << "docid: " << it->docids[i] << ", "
                    << "hitcount: " << it->hitcounts[i] << '}';
            }

...
public:
    Paginator(Iterator f, Iterator l, size_t psize)
        : page_size(psize)
    {
        auto first = f;
        size_t step = 0;
        for (first = f; first < l; first += step) {
            step = (l - first >= page_size) ? page_size : (l - first);
            pages.push_back(IteratorRange(first, first + step));
        }
    }

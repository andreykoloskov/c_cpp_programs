...
public:
    ...
    using Array = array<Item, static_cast<size_t>(TAirport::Last_)>;
    
    class ItemsInner
    {
    public:
        ItemsInner(const Array& arr): airports_inner(arr) { }
        auto begin() { return airports_inner.begin(); }
        auto end() { return airports_inner.end(); }
        const auto begin() const { return airports_inner.begin(); }
        const auto end() const { return airports_inner.end(); }
        const Array& airports_inner;
    };

    using Items = ItemsInner;

    // конструктор по умолчанию: список элементов пока пуст
    AirportCounter()
    {
        for (auto i = 0; i < static_cast<uint32_t>(TAirport::Last_); ++i) {
    ...
    AirportCounter(TIterator begin, TIterator end)
    {
        ...
        for (TIterator i = begin; i != end; ++i) {
            auto i0 = static_cast<size_t>(*i);
            airports[i0].second += 1;
        }
    }
